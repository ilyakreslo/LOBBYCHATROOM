...
schema.statics.addRoomToUsers = function (roomId, roomName, usernames, callback) {
    var User = this;
    console.info("Adding users " + usernames + " room (" + roomId + "," + roomName + ").");

    User.find({username: {$in: usernames}}, function (err, users) {
        if (err) return callback(err);

        console.info("Users found: ", users.length || 0);
        ...
        console.info("Users who need to add a room: ", users);
        ...
        users.forEach(function (user, index) {
            ...
            users[index].save(function(err, usr) {
                if (err) return console.error("An error occurred while adding the room to user %s : %s", usr, err);
            });
...
schema.statics.leaveRoom = function (roomName, username, callback) {
    ...
    User.findOne({username: username}, function (err, user) {
        if (err) return callback(err);
        if (!user) return callback("User " + username + " not found");

        var index = utils.indexOfObjByAttr(user.rooms, "roomName", roomName);
        if (index < 0) return callback("Room " + roomName + " not found by user " + username);
        ...
        user.save(function(err) {
            ...
            Room.findById(roomId, function(err, room) {
                if (err) return callback(err);
                if (!room) return callback("Room " + roomName + " с id=" + roomId + " not find in collections Rooms.");
                ...
                if (index > -1) {
                    ...
                    return callback("In room " + roomName + " с id=" + roomId + " not find user " + username);
                }
            })
        })
