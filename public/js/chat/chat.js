...
socket
    ...
    .on('invited', function (room) {
        ...
        printServerMessage("<i>You were invited to the room </i><b>" + room.roomName + "</b>");
    })
    .on('userLeave', function (username, roomId) {
        if (roomId === roomsList.currentRoom._id) {
            printServerMessage("<i>User <b>" + username + "</b> left the room. </i>");
        }
    })
...
function sendMessage() {
    ...
    socket.emit('message', text, function () {
        ...
        messageContainer.addText("<p><b>Me</b>: " + wrapWithClass(safeString, "mymsg") + "</p>");
        receivedMessages.addMessage(roomsList.currentRoom._id, {username: "Me", message: safeString});
    });
...
function setUserStatusOnline (username, isOnline) {
    ...
    var msg = isOnline ? " joined the chat." : " left the chat.";
